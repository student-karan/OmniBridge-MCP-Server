...
import { BranchSummarySchema } from "../../utils/types.js";
...
export async function createBranch(args) {
    ...
    try {
        ...
        await logInteraction("create_branch", logInput, `Branch '${args.branch}' successfully created in ${args.repo}.`, "success", repo_owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("create_branch", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
...
export async function deleteBranch(args) {
    ...
    try {
        ...
        await logInteraction("delete_branch", logInput, `Branch '${args.branch}' successfully deleted from ${args.repo}.`, "success", repo_owner, targetRepo);
        return { message: `Branch '${args.branch}' deleted successfully.` };
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("delete_branch", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
...
export async function listBranches(args) {
    ...
    try {
        ...
        await logInteraction("list_branches", logInput, `Successfully fetched ${data.length} branches from ${args.repo}.`, "success", repo_owner, targetRepo);
        return data.map(branch => BranchSummarySchema.parse(branch));
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("list_branches", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
// Function to get specific branch details
export async function getBranch(args) {
    ...
    try {
        ...
        await logInteraction("get_branch", logInput, `Successfully fetched details for branch '${args.branch}' in ${args.repo}.`, "success", repo_owner, targetRepo);
        return BranchSummarySchema.parse(data);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("get_branch", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
