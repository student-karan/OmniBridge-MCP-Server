...
import { NotificationSchema } from "../../utils/types.js";
...
export async function listNotifications(args) {
    ...
    try {
        ...
        await logInteraction("list_notifications", logInput, `Notifications for the ${repo_owner} are successfully fetched.`, "success", repo_owner);
        ...
        await logInteraction("list_notifications", logInput, errorMsg, "error", repo_owner);
        throw err;
    }
}
// Function to mark user specific notification as read.
export async function MarkNotificationRead({ thread_id }) {
    ...
    try {
        ...
        await logInteraction("mark_notification_read", logInput, `${repo_owner} Notification ${thread_id} is successfully marked as read.`, "success", repo_owner);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occured.";
        await logInteraction("mark_notification_read", logInput, errorMsg, "error", repo_owner);
        throw err;
    }
}
