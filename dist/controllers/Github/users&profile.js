...
export async function getMyProfile() {
    ...
    try {
        ...
        await logInteraction("get_my_profile", logInput, "User profile data is successfully fetched.", "success", repo_owner);
        return UserProfileSchema.parse(data);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occured.";
        await logInteraction("get_my_profile", logInput, errorMsg, "error", repo_owner);
        throw err;
    }
}
// Function to list all of the user followers 
export async function listFollowers({ per_page, page }) {
    ...
    try {
        ...
        await logInteraction("list_followers", logInput, "User's followers names are successfully fetched.", "success", repo_owner);
        return data.map(user => user.login);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occured.";
        await logInteraction("list_followers", logInput, errorMsg, "error", repo_owner);
        throw err;
    }
}
// Function to list all Users followed by the user  
export async function listFollowing({ per_page, page }) {
    ...
    try {
        ...
        await logInteraction("list_following", logInput, "The names of the users followed by the user are successfully fetched.", "success", repo_owner);
        return data.map(user => user.login);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occured.";
        await logInteraction("list_following", logInput, errorMsg, "error", repo_owner);
        throw err;
    }
}
...
export async function getUser({ username }) {
    ...
    try {
        ...
        await logInteraction("get_user", logInput, `The details of ${username} are successfully fetched.`, "success", username);
        return UserProfileSchema.parse(data);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("get_user", logInput, errorMsg, "error", username);
        throw err;
    }
}
