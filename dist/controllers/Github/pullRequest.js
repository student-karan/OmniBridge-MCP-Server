...
import { PullRequestSummarySchema } from "../../utils/types.js";
...
export async function createPullRequest(args) {
    ...
    try {
        ...
        await logInteraction("create_pull_request", logInput, `The pull request with number : ${data.number} is successfully created.`, "success", repo_owner, targetRepo);
        return PullRequestSummarySchema.parse(data);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("create_pull_request", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
...
export async function mergePullRequest(args) {
    ...
    try {
        ...
        await logInteraction("merge_pull_request", logInput, `Pull request with number : ${args.pull_number} is successfully merged with the repo ${args.repo}.`, "success", repo_owner, targetRepo);
        return data; // Merging returns a message/sha, not the full PR object
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("merge_pull_request", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
...
export async function closePullRequest(args) {
    ...
    try {
        ...
        await logInteraction("close_pull_request", logInput, `Pull request with number : ${args.pull_number} is successfully closed in the repo ${args.repo}.`, "success", repo_owner, targetRepo);
        return PullRequestSummarySchema.parse(data);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("close_pull_request", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
...
export async function listPullRequests(args) {
    ...
    try {
        ...
        await logInteraction("list_pull_requests", logInput, `Successfully fetched ${data.length} pull requests from ${args.repo}.`, "success", repo_owner, targetRepo);
        return data.map(pr => PullRequestSummarySchema.parse(pr));
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("list_pull_requests", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
...
export async function getPullRequest(args) {
    ...
    try {
        ...
        await logInteraction("get_pull_request", logInput, `Successfully fetched details for PR #${args.pull_number} in ${args.repo}.`, "success", repo_owner, targetRepo);
        return PullRequestSummarySchema.parse(data);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("get_pull_request", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
// Function to add a comment to a pull request
export async function addPullRequestComment(args) {
    ...
    try {
        ...
        await logInteraction("add_pr_comment", logInput, `Successfully added a comment to PR #${args.pull_number} in ${args.repo}.`, "success", repo_owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An unknown error occurred.";
        await logInteraction("add_pr_comment", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
