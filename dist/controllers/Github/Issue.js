...
export async function createIssue(args) {
    ...
    try {
        ...
        await logInteraction("create_issue", logInput, `Issue #${data.number} created in ${args.owner}/${args.repo}`, "success", args.owner, targetRepo);
        return {
            id: data.id,
            number: data.number,
            title: data.title,
        ...
        await logInteraction("create_issue", logInput, errorMsg, "error", args.owner, targetRepo);
        throw err;
    }
}
...
export async function closeIssue(owner, repo, issue_number) {
    ...
    try {
        ...
        await logInteraction("close_issue", logInput, `Issue "${issue_number}" closed in ${owner}/${repo}.`, "success", owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while closing the issue.";
        await logInteraction("close_issue", logInput, errorMsg, "error", owner, targetRepo);
        throw err;
    }
}
...
export async function updateIssue(args) {
    ...
    try {
        ...
        await logInteraction("update_issue", logInput, `Issue "${args.issue_number}" updated in ${args.owner}/${args.repo}.`, "success", args.owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while updating the issue.";
        await logInteraction("update_issue", logInput, errorMsg, "error", args.owner, targetRepo);
        throw err;
    }
}
// Function to list all issues in a repo
export async function listAllIssues(args) {
    ...
    try {
        ...
        await logInteraction("list_all_issues", logInput, `Issues listed from ${args.owner}/${args.repo}.`, "success", args.owner, targetRepo);
        return list;
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while listing the issues.";
        await logInteraction("list_all_issues", logInput, errorMsg, "error", args.owner, targetRepo);
        throw err;
    }
}
// Function to get details of specific issue
export async function getIssueDetails(owner, repo, issue_number) {
    ...
    try {
        ...
        await logInteraction("get_issue_details", logInput, `Issue "${issue_number}" in ${owner}/${repo}. is successfully fetched.`, "success", owner, targetRepo);
        return {
            id: data.id,
            number: data.number,
            title: data.title,
        ...
        await logInteraction("get_issue_details", logInput, errorMsg, "error", owner, targetRepo);
        throw err;
    }
}
// Function to add a comment for an issue
export async function addIssueComment(owner, repo, issue_number, comment) {
    ...
    try {
        ...
        await logInteraction("add_issue_comment", logInput, `Comment added in issue "${issue_number}" of Repo ${owner}/${repo}`, "success", owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || `An error occurred while creating an comment on an issue of repository ${owner}/${repo} .`;
        await logInteraction("add_issue_comment", logInput, errorMsg, "error", owner, targetRepo);
        throw err;
    }
}
