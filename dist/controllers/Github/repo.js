...
import { RepoDetailsSchema, RepositorySchema } from "../../utils/types.js";
import repoExists, { extractErrorMessage } from "../../utils/utility.js";
// Function to create a new Repo
export async function createRepo(repo, visibility, desc) {
    ...
    try {
        ...
        await logInteraction("create_repo", logInput, `Repository with name : ${repo} was successfully created`, "success", repo_owner, BigInt(repository.data.id));
        ...
        await logInteraction("create_repo", logInput, errorMsg, "error", repo_owner, null);
        throw err;
    }
}
...
export async function deleteRepo(repo) {
    ...
    try {
        ...
        await logInteraction("delete_repo", logInput, `Repository with name : ${repo} was successfully deleted`, "success", repo_owner, BigInt(check.id));
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while deleting the repository.";
        await logInteraction("delete_repo", logInput, errorMsg, "error", repo_owner, null);
        throw err;
    }
}
// Function to update a Repo metadata
export async function updateRepoMetadata(repo, newName, desc) {
    let targetRepo = null;
    ...
    try {
        ...
        await logInteraction("update_repo_metadata", logInput, `Metadata of repository with name : ${repo} was successfully updated.`, "success", repo_owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while updating the repository description.";
        await logInteraction("update_repo_metadata", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
// Function to change the repository visibility 
export async function changeRepoVisibility(repo, visibility) {
    let targetRepo = null;
    ...
    try {
        ...
        await logInteraction("change_repo_visibility", logInput, `Repository visibility of : ${repo} was successfully changed to : ${visibility}`, "success", repo_owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while updating the repository visibility.";
        await logInteraction("change_repo_visibility", logInput, errorMsg, "error", repo_owner, targetRepo);
        throw err;
    }
}
...
export async function listAllRepos() {
    ...
    try {
        ...
        await logInteraction("list_all_repos", logInput, `Found ${repoNames.length} repositories.`, "success", repo_owner, null);
        return repoNames;
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while listing the repositories.";
        await logInteraction("list_all_repos", logInput, errorMsg, "error", repo_owner, null);
        throw err;
    }
}
// Function Get the details of a specific repo
export async function getRepoDetails(owner = repo_owner, repo) {
    let targetRepo = null;
    ...
    try {
        ...
        await logInteraction("get_repo_details", logInput, `Details of repository : ${owner}/${repo} fetched successfully.`, "success", owner, targetRepo);
        ...
        await logInteraction("get_repo_details", logInput, errorMsg, "error", owner, targetRepo);
        throw err;
    }
}
// Function to fork a Repo
export async function forkRepo(owner = repo_owner, repo, my_fork_name) {
    let targetRepo = null;
    ...
    try {
        ...
        await logInteraction("fork_repo", logInput, `Successfully forked ${owner}/${repo}.`, "success", owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while forking a repository.";
        await logInteraction("fork_repo", logInput, errorMsg, "error", owner, targetRepo);
        throw err;
    }
}
// Function to star a Repo
export async function starRepo(owner = repo_owner, repo) {
    let targetRepo = null;
    ...
    try {
        ...
        await logInteraction("star_repo", logInput, `Successfully starred ${owner}/${repo}.`, "success", owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while starring a repository.";
        await logInteraction("star_repo", logInput, errorMsg, "error", owner, targetRepo);
        throw err;
    }
}
// Function to unstar a Repo
export async function unStarRepo(owner = repo_owner, repo) {
    let targetRepo = null;
    ...
    try {
        ...
        await logInteraction("unstar_repo", logInput, `Successfully unstarred ${owner}/${repo}.`, "success", owner, targetRepo);
    }
    catch (err) {
        let errorMsg = extractErrorMessage(err) || "An error occurred while un-starring a repository.";
        await logInteraction("unstar_repo", logInput, errorMsg, "error", owner, targetRepo);
        throw err;
    }
}
